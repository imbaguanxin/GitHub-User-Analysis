...
user_readable_df %>% filter(following > 3000) %>% select (login, id, following) %>% arrange (desc(following))
...
# watchers count, language, has pages, forks count, archived, disabled,
# open issues count, license.
...
find_repo <- function(repo_id, db = db.repo){
  ...
  if(repo_df$license %>% length() > 0){
    if("name" %in% names(repo_df$license)){
      repo_df$license <- repo_df$license$name
    } else{
      repo_df$license <- repo_df$license %>% toString()
    }
  } else {
    repo_df$license <- NA
  }
...
find_user_repo(33470168)
temp <- find_repo(173036713)
```
